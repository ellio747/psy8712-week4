...
wide_tbl[, c("q1", "q2","q3","q4", "q5")] <- sapply(wide_tbl[, c("q1", "q2","q3","q4", "q5")], as.integer) #Note: one response to q4 was a 6. Maybe a response issue.
wide_tbl <- mutate(wide_tbl, datadate = mdy_hms(datadate)) # converts to POSIXct format; verified in class(wide_tbl$datadate)
wide_tbl <- mutate(wide_tbl, across(q1:q5, ~ na_if(., 0))) # 18 NA in place of 0
wide_tbl <- drop_na(wide_tbl, q2) # will remove 9 subjects with NA for q2; sum(is.na(wide_tbl[, 5:9]))
